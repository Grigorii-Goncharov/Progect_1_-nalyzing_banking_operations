...
def get_currency(path_to_json: str) -> list[dict]:
    ...
    logger.info(
        "Работа функции get_currency: Осуществляю вывод топ-транзакций по сумме платежа... ОК"
    )
...
def get_stock(path_to_json: str) -> list[dict]:
    ...
    with open(path_to_json, "r", encoding="utf-8") as file:
        ...
        for stock in stocks:
            ...
            # сохраняем статус-код ответа в переменную status_code
            status_code = response.status_code
            logger.info("Проверяю статус код...")
            if status_code == 200:
                logger.info("Проверяю статус код... ОК")
                data = response.json()
                stock_rates.append({"stock": symbol, "price": data["price"]})

    logger.info("Работа функции get_stock Осуществляю вывод курса акций... ОК")
    return stock_rates
